...
We will see the mathematical model, how to calculate the data we need ($\theta_{max}$ and period) using raw data obtained by the accelerometer.
...
We calculated the errors according to the values obtained using 3 members of the series in table \textbf{CITE}.  
...
As we will see the max angle of the pendulum is of $\frac{\pi}{2}$ for that value the error is just of 1.8\%, an acceptable value.

%say that we can use one term and we don't need theta, is an observation
%say how to calculate theta and  period
%say the error in % using one term
%say int he next chapter that for a lot of reasons the experiment is doen using large amplitude, say why
%say the reasons of the angle choosen
